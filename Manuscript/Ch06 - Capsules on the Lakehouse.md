...
This convergence is the good news. Instead of bolting a separate metadata system onto the side of your platform, you can lean on what the table format already provides. The table's transaction log, its schema definition, its partition spec, and its table properties form a natural anchor point. Everything else — catalogues, documentation portals, lineage graphs — can *reference that anchor* rather than competing to be the source of truth.
...
A fair objection from a large part of the audience: *we have no table format — we are a Snowflake or BigQuery shop, and our data lives in a warehouse, not a lakehouse.* The discipline still applies; it simply binds to different primitives. A warehouse gives you the same anchor points under other names — table and column comments, object tags and policy tags, the information schema, native lineage — and the hot/cold split below maps onto them directly. Chapter 11 covers those platform-native hooks (Snowflake tags, Unity Catalog, BigQuery policy tags) in detail. The point to carry here is that the capsule abstraction sits *above* the storage layer: lakehouse or warehouse, the unit of design is the same, and only the binding mechanics differ.
...
- **Policy metadata** → classification, legal basis, retention, and masking expressed as table properties and, where the catalogue supports it, as enforced tags and masking policies.
...
<!-- FIG 6.1: hot vs cold metadata — hot (schema, column stats, primary classification, partition info) embedded in the table; cold (full lineage graphs, documentation, audit history, cost analyses) referenced by stable, co-versioned identifier. -->
...
### Pattern 3: Catalogues are indexes, not sources of truth

Unity Catalog, AWS Glue, Project Nessie, Apache Polaris, Collibra, Atlan — these are valuable for discovery and access control, and you should use them. But they must function as an *index over* capsules, not as the authoritative source of table metadata. The real metadata lives in the table format, in object storage, next to the data. The catalogue provides a searchable, queryable window onto that reality.

The test of whether you have this right is what happens during an argument. If the catalogue says one thing and the table's own metadata says another, **the table wins**, and the catalogue needs refreshing or repairing. A team that resolves that disagreement the other way — trusting the catalogue over the data — has rebuilt the drift problem with a more expensive tool. This single principle, applied consistently, is what turns a catalogue from a documentation graveyard into a live index of the truth.
...
Here the snapshot ID is the lineage linchpin: OpenLineage records which job produced which snapshot, and quality runs gate snapshot promotion — tests must pass before a new snapshot becomes the current version consumers see. **The gotcha for Iceberg is catalogue choice.** Hive Metastore, Glue, Nessie, and Polaris differ in their support for tags and access control, so pick the catalogue whose governance features you actually need, because the table format alone will not provide column masking.
...
The five-step migration reads cleanly, so here is how Meridian's first one — wrapping the legacy `client_holdings` extract — actually went, because the friction is the instructive part.
...
## Choosing a catalogue (it matters more than you expect)

The chapter's earlier Iceberg note flagged that catalogue choice matters; here is the comparison, because for an Iceberg or multi-engine shop the catalogue decides which governance features the capsule can actually enforce. The table format gives you schema and snapshots; the *catalogue* is what gives you tags, masking, and access control.

| Catalogue | Multi-engine | Tags / classification | Column masking / ABAC | Notes |
|---------|:---:|:---:|:---:|-------|
| Hive Metastore | ✓ | limited | ✗ | Ubiquitous, minimal governance |
| AWS Glue | ✓ | ✓ (Lake Formation) | ✓ (Lake Formation) | Good on AWS; ties you to it |
| Unity Catalog | Spark-centric\* | ✓ | ✓ | Rich governance; historically Databricks-centric, broadening |
| Project Nessie | ✓ | via properties | ✗ | Git-like branching for data (Dremio-backed) |
| Apache Polaris | ✓ | ✓ | emerging | Open Iceberg REST catalogue (incubating) |
| Iceberg REST + engine | ✓ | via properties | engine-dependent | Maximum portability, least built-in governance |

\*Catalogue capabilities move fast: Unity Catalog's 2024 open-sourcing and Iceberg REST support are broadening its engine reach well beyond Spark/Databricks. Verify against current releases; this comparison was checked in mid-2026.

The rule of thumb: **choose the catalogue for the governance features you need to enforce, not the format for them** — because the format is largely interchangeable and the catalogue is where masking and access control actually live. Meridian runs Iceberg with a catalogue that supports tag-based masking, because `client_id` masking is a policy the platform must enforce, not a convention the capsule merely declares.

## What the discipline costs at 50 TB

Column statistics, constraints, and pre-commit validators are not free at scale. On a 50 TB capsule, computing full column statistics on every write is expensive, so Meridian samples for distribution checks and computes exact stats only on partitions touched by the write. Pre-commit validators add latency to each commit, which matters for the intraday Hudi feed and barely registers for the daily holdings snapshot — so the *depth* of validation is tiered to the write cadence. And CI table-deployment must stay fast enough that engineers do not route around it, which means running the heavy quality suite against a *sampled* staging snapshot in the PR and the full suite on a schedule. The governing principle mirrors the hot/cold split: spend validation effort in proportion to blast radius and cadence, so the discipline stays affordable and therefore stays used.
...
The common thread survives whichever you pick: define tables as code, store metadata in the table rather than only in the catalogue, co-locate contracts and tests, version everything together. You can even mix formats across an organisation — the capsule abstraction sits above the table format. What matters is that each table is self-describing, governed, and trustworthy.

## The shift, restated

The mental move this chapter asks for is to stop treating `CREATE TABLE` as a one-time setup command and start treating it as **the primary place your Data Capsule lives.** The table definition in Git — schema, comments, properties, references to contracts and tests — is the source of truth. The catalogue is an index. The documentation portal is a view. The governance dashboard is a report. Get that ordering right and the implementation steps write themselves: pick an important table, define it completely, deploy it through CI/CD, monitor it, iterate.

The payoff compounds. Each capsule is one fewer dataset that can surprise you, one fewer "I thought someone else owned that," one fewer compliance gap to explain to an auditor — and one more piece of your estate that is ready for whatever comes next. For Meridian, "whatever comes next" was not hypothetical. It was an adviser copilot in production and a suitability model heading for a model-risk committee — both consuming these tables not as dashboards for humans but as machine-read inputs with regulatory consequences. Batch tables sitting quietly in object storage are only half the picture. The next chapter takes the capsule discipline into the half where the stakes are highest: training data, feature stores, and data in motion.

## Further reading

- The Apache Iceberg, Delta Lake, and Apache Hudi specifications and documentation — the primary sources for the format mechanics compared here.
- OpenLineage (openlineage.io) for the lineage-event standard that links each snapshot to the job that produced it (built into a platform in Chapter 13).
- On catalogues and governance features, the Unity Catalog, AWS Glue / Lake Formation, Project Nessie, and Apache Polaris documentation — verify current capabilities, which move quickly.

> **Chapter summary.** The lakehouse table format is the natural home for a capsule because data and metadata already converge there and move atomically in versioned commits. Map the seven components onto the format's schema, properties, comments, snapshots, and constraints; split hot metadata (embedded) from cold metadata (referenced). Three patterns hold across Delta, Iceberg, and Hudi: the table is an atomic object, table definitions are code, and catalogues are indexes rather than sources of truth. Migrate incrementally, one high-value dataset at a time. The format choice matters less than the discipline.
