# Chapter 10 — Metadata as Code: GitOps for Governance
...
The 7 a.m. failure is a specific instance of a general condition: the velocity and volume of change in a modern data estate have outrun the human-scale processes meant to govern it. Each way it breaks is a thing Metadata as Code is designed to fix.
...
**Documentation drift.** The hand-maintained catalogue falls out of date within months — Chapter 3's latency problem, restated operationally: at scale, schemas change faster than any human can document them.
...
Now replay the change that *actually caused* the Okonkwo error — the enrichment job quietly applying the institutional basis to retail rows — in a world where that logic lives in this repository instead of in an untracked job edit. The engineer's change touches `pipelines/`, and because the affected output field carries a glossary reference, CI checks the edited logic against `glossary/wealth/emerging_markets.yaml` and flags a semantic contradiction: the job would now write institutional-basis values into a field whose contract declares the retail definition. `CODEOWNERS` — because the change's blast radius reaches `glossary/wealth/` and `contracts/wealth/` — automatically requests review from Maya (the product owner) and the reviewer who understands both divisions. Impact analysis lists the copilot and three reports as affected; the reviewer sees the contradiction *before* it reaches production — the exact human check that was missing when the change slipped through as a silent enrichment-job edit. The change that once reached production as an invisible job edit is now a visible, contested diff, and it does not ship as written.

(The *eventual* fix — redefining `em_exposure_pct` to be division-aware, release 4.0.0 in Chapter 5's version history — travels the same road, but as a *declared* major change: the author records the semantic redefinition, CI classifies it major and demands the 90-day consumer notice, and it deploys with the copilot's normalisation updated in lockstep. Semantic contradictions are surfaced by declared references and human review, not by magic diffing — the point Chapter 6 made about what CI can and cannot compute. Either way, the Okonkwo failure is caught at the pull request, which is the chapter's central promise, shown once and concretely.)

### The CI stages

The pipeline that stands behind the PR runs a fixed sequence of stages, and each is a guarantee:
...
Effective Metadata as Code rests on four principles, and every chapter in Part III elaborates one or more of them.
...
And crucially, this is how the capsule discipline *scales*. Binding one capsule by hand proved the idea (Part II). Metadata as Code is what lets Meridian have a hundred capsules without a hundred times the manual effort, because the binding is now declarative, templated, and enforced by CI/CD. Helena, who runs Meridian's catalogue programme, no longer maintains the catalogue by hand against a moving estate; the catalogue becomes a consumer of these specifications, refreshed from the source of truth rather than curated in parallel with it — a relationship we will make precise in Chapter 14. Tom, who runs the data-quality programme, no longer chases quality after the fact; his expectations become tests in the repository that gate deployment. Governance stops being a meeting and starts becoming a property of the pipeline.

That is the shift, and it sets up the rest of Part III. This chapter argued the principle: treat metadata as a versioned, tested, declarative product, governed like code. The next chapter makes it concrete, walking through the open standards and platform features — OpenLineage, dbt artefacts, AsyncAPI, Great Expectations, and the native governance hooks of the major platforms — that turn the principle into a working stack. Then Chapter 12 brings the third principle, event-driven automation, to its full conclusion: metadata that does not merely sit in Git correctly, but flows through the platform and *acts*.

## Further reading

- On the GitOps and Infrastructure-as-Code lineage this chapter borrows from, the Terraform documentation and the GitOps principles published by the OpenGitOps project.
- On the data-side toolchain, the dbt (artefacts and tests), Great Expectations, and OpenLineage documentation — all developed in Chapter 11.
- The *Open Data Contract Standard* (Bitol / LF AI & Data) for the contract specs that sit in the `contracts/` path of the repository above.
