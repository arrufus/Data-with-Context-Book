# Chapter 13 — Standards-Based Metadata Infrastructure

> **Where you are:** Part III — *Automate*. Chapter 12 made metadata act. But active metadata across a sprawling estate only works if every tool can speak a common language and share a common graph. This chapter builds that substrate — the metadata integrator — using open standards and a knowledge graph. Maturity: this is what **enables Level 3 across a multi-tool estate**, not just within one platform. It is the spine beneath the nervous system.
...
**Query trust metrics.** The platform tracks freshness, quality, and ownership and exposes a trust score, so consumers — human and agent — know what to rely on before they build on it. This is the data behind Chapter 12's dynamic trust scoring.

## Three design decisions worth understanding

Most of this architecture is assembly, but three choices are load-bearing enough to explain, because they are what make the platform answer the questions that matter to a regulated firm.
...
**Access control on the graph itself.** Chapter 5 warned that some metadata is more sensitive than the data it describes — internal topology, the structure of controls, contractual constraints. Centralising all of it into one queryable graph therefore raises its own governance question: *who may query what?* The integrator answers it the same way the products do. Graph access is role-scoped, so a SPARQL query or API call returns only the facets the caller is entitled to; sensitive facets — control topology, security lineage, classification of the metadata itself — are restricted rather than open by default; and the audit log covers metadata *reads*, not just writes, because a record of who inspected the lineage of a regulated product is itself evidence. The integrator is the highest-value target in the architecture precisely because it connects everything, so it is governed at least as strictly as the data it indexes.

### Bitemporality, shown

The bitemporal idea is abstract until you see the two timelines on one fact. Here is an ownership change on `client_holdings` — Maya inherited the product from a predecessor in mid-2025 — recorded so the past can be reconstructed as it was *understood at the time* (shown here as classic RDF reification for portability; RDF-star is the cleaner encoding where your store supports it):

```turtle
:ownership_v1 rdf:subject :client_holdings ; rdf:predicate :hasOwner ;
    rdf:object "daniel.roche@meridian.example" ;          # Maya's predecessor
    :validFrom "2024-01-01" ; :validTo "2025-06-01" ;      # true in the world
    :transactionTime "2024-01-01T09:00:00Z" .              # when we recorded it
:ownership_v2 rdf:subject :client_holdings ; rdf:predicate :hasOwner ;
    rdf:object "maya.osei@meridian.example" ;
    :validFrom "2025-06-01" ; :validTo :null_high ;
    :transactionTime "2025-06-01T11:00:00Z" .
```

And the query that answers the audit question from above — *who owned this dataset on a given past date?* — asked here for a date in early 2025:

```sparql
SELECT ?owner WHERE {
  ?s rdf:subject :client_holdings ; rdf:predicate :hasOwner ; rdf:object ?owner ;
     :validFrom ?from ; :validTo ?to .
  FILTER(?from <= "2025-03-01" && ?to > "2025-03-01")
}
```

A single-state store answers "Maya" — today's owner — which is wrong for an early-2025 date. The bitemporal store answers her predecessor, correctly, because it kept both timelines. Run the *same* query for the model's training date, 2026-03-15, and it correctly returns Maya — the reconstruction Part V relies on when it decides whether a model survives its audit. Either way, the answer can be "here it is" rather than "I'll come back to you."
...
But one note bears repeating, because Part III keeps arriving at it: technology gives the structure; organisational commitment gives it life. DCAT and SHACL do not enforce themselves, and lineage does not stay accurate without engineers who care about precision. The teams that succeed invest as much in education as in infrastructure, because a platform that nobody understands is a platform that nobody feeds. Meridian now has the substrate: a connected, standards-based, queryable, bitemporal metadata graph that lets active metadata act across the whole estate rather than within one tool. What it does not yet have is a disciplined way to *roll this out* — to take the capsule, the code, the active layer, and the integrator and adopt them across a real organisation without trying to do everything at once, and without falling into the trap of letting the shiny new platform become yet another competing source of truth. That discipline — capsules as the source, platforms as consumers, adopted phase by phase — is the final chapter of Part III, and it is where the whole *Automate* move becomes an operating reality rather than an architecture diagram.

## Further reading

- On lineage capture, the OpenLineage specification (openlineage.io).
- On the semantic layer, the W3C standards this chapter builds on: DCAT (Data Catalog Vocabulary), PROV-O (Provenance Ontology), SKOS (Simple Knowledge Organization System), and SHACL (Shapes Constraint Language); on the newer reification encoding, the RDF-star / SPARQL-star drafts.
- On storage, the Apache Jena/Fuseki, Ontotext GraphDB, and Amazon Neptune documentation for triple stores at the scales discussed here.
