...
With the ontology defined and the graph populated, assembly brings the five layers of Chapter 14 together into a coherent product: the **base data product** (the governed `client_holdings` dataset), the **ontology layer** (entities, relationships, rules), the **semantic layer** (business-friendly mappings and calculation logic), the **knowledge graph** (queryable connections, lineage, confidence), and **usage intelligence** (documented patterns, limitations, examples).

Exposure matters as much as composition. The product must be reachable through a **semantic query API** that agents and analytics tools call programmatically; **discoverable in the enterprise catalogue** with its full contextual metadata, not buried in a separate system; and governed by a **data contract extended to include semantic obligations** — the ontological definitions, the confidence-scoring commitments, the usage documentation — alongside the traditional schema and quality terms. Meridian's suitability contract now promises not just fresh, complete holdings but *this* definition of EM exposure, *this* confidence signal, *these* permitted uses.

What "done" looks like: the copilot can query the suitability domain and receive a contextually enriched answer, with ambiguous terms — frontier-market inclusion, risk-band interpretation — resolved *programmatically*, not by a human. The product carries lineage, confidence scoring, and at least three documented usage patterns. The Adeyemi question from Chapter 14 now returns a correct, defensible answer, because the rule that resolves it is in the product and served through the API.
...
Package the results into a **one-page value summary** for leadership, and be honest about what was harder than expected — credibility matters more than spin, and a leadership that trusts the first summary funds the second domain. Identify the next two candidate domains based on what the team learned. And propose a **lightweight context standard**: the minimum semantic enrichment every data product should carry going forward — the seed of scaling from one product to a discipline.
