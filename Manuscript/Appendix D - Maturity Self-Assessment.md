...
*Two instruments you can run on your own estate: the per-asset maturity ladder (0–4) that measures the four-move climb, and the six-surface evidence instrument from Chapter 20 that measures whether a product can testify. Together they turn "AI-ready" from a feeling into a pair of numbers you can track over a rollout.*
...
| 2 | **Modelled** | an ontology exists for its domain; entities & relationships documented | Bind + model (Ch 8, 17) |
| 3 | **Connected** | a knowledge graph ties entities, lineage & confidence; ≥1 AI consumer live | Automate + Package (Ch 12, 16) |
| 4 | **Intelligent** | usage intelligence, dynamic confidence scoring, semantic APIs; context standards enforced | Prove + scale (Ch 19, 21) |

**How to use it.** Score every tier-1 product, tabulate the distribution, and track it quarterly. The number that matters most is the *falling count of Level 0–1 products* — that is the rollout working. Targets by quarter (from Chapter 14): pilot cohort to L3 in Q1; 20% of tier-1 at L2+ by Q2; 50% by Q3; all *new* products born at L1+ by Q4.

**Interpretation bands (portfolio view):**
- *Most of the estate at 0–1* — normal starting point; you have a discovery-and-trust problem and are not AI-ready. Pick one high-value product and run the 90-day playbook (Ch 16).
- *A pilot cohort at 3, the rest at 0–1* — healthy early rollout. The risk now is stalling (Ch 14's "stall" case); make domain owners accountable and fund the next domains from the pilot's value summary.
...
The maturity ladder measures *capability*; the evidence instrument measures whether a product can *testify* — the Chapter 18/19 test. Score each surface **0 / 1 / 2**:
...
**The Chapter 4 quick check, mapped.** The ten questions Chapter 4 poses (scored 0/1/2, out of 20) are the fast, informal preview of this instrument; they probe the same six surfaces — ownership (Q1), metadata/meaning (Q2), quality (Q3), lineage (Q4–5), privacy/lawful basis (Q6), consumption (Q7), plus the change-notification, certification, and operational-detection questions (Q8–10) that Appendix C's certification model develops. Use the ten questions for a two-minute gut check and this six-surface score for a tracked assessment; they are the same diagnosis at two resolutions.
...
|---------|:---:|:---:|---------------------|
| Ownership | 1 | 2 | named owner with decision rights + pager (Ch 21) |
| Metadata | 0 | 2 | semantics bound as code; glossary refs (Ch 5) |
| Quality | 1 | 2 | contracted, per-use, gating tests (Ch 6, 11) |
| Lineage | 0 | 2 | OpenLineage + bitemporal graph (Ch 13) |
| Privacy | 0 | 2 | lawful-basis manifest, runtime masking (Ch 7, 11) |
| Consumption | 1 | 2 | registered, contracted consumers (Ch 14) |
| **Total** | **3** | **12** | |
...
4. **Re-score quarterly** — the trend is the point. Falling Level-0/1 counts and rising evidence totals are what you show leadership to fund the next domain (Ch 22).
5. **Be honest.** The instrument is worthless if scored generously; a "1 we call a 2" is exactly the monitoring-mistaken-for-evidence error that pauses models in Chapter 20.
