...
> **Where you are:** Part II — *Bind*. Chapters 5–7 bound context to data; this chapter addresses the structure that makes context possible to bind in the first place, and Chapter 9 then carries the discipline into unstructured data. Together they set up the move from *Bind* to *Automate* and *Package*. Maturity note: rigorous modelling is the bridge from **Level 1 (Defined)** toward **Level 2 (Modelled)**.
...
Two more consequences follow, and they are the budget arguments.
...
<!-- FIG 8.1: medallion × modelling grid — Bronze (raw, structural/provenance metadata), Silver (integration: 3NF default or Data Vault for multi-source regulated domains), Gold (consumption: star schema for humans, wide serving table for machines); shows which modelling approach belongs in each layer. -->
...
This is a two-statement idiom: the MERGE above closes the changed row, and a second pass (omitted for space) inserts the new current row for the clients whose attributes changed, because a single MERGE cannot both close and open a row for the same key. Together they close the old row and open a new one on change, which is exactly what lets the copilot do the *point-in-time* join Chapter 7's temporal-correctness discussion demanded — reading a client's division *as it was on the assessment date*, not as it is today.
...
Second, **the semantic layer is where the model stops being a set of tables and starts being a set of business concepts** — and it is the natural seam between Part II and Part III. A semantic layer (dbt's MetricFlow, LookML, or a dedicated tool) sits above Gold and maps physical tables to business concepts: "active client," "emerging-market exposure," "suitability breach," each with one governed definition and one calculation. Modelled well, it serves humans and agents from the same trusted concepts, so an analyst's "EM exposure" and the copilot's "EM exposure" are guaranteed to be the same thing. This is the foreshadow of the **context data product** that Part IV builds: a base data product, plus an ontology of entities and relationships, plus a semantic layer of governed concepts, plus the knowledge graph and usage intelligence that make it consumable by reasoning systems. Modelling is where that stack gets its foundation. Part III will keep the bound context correct at scale; Part IV will package it — and you cannot package context you have not first structured.
...
## The structure beneath the binding

That puts the modelling prerequisite in place. Chapter 5 named the unit — the Data Capsule, data and metadata as one co-versioned, deployable whole. Chapter 6 gave it a home in the lakehouse table and the patterns that keep it honest. Chapter 7 carried it into the high-stakes world of training data, features, and streams, and showed the payoff at its sharpest: a model that survives its audit because its data can produce evidence about itself. This chapter supplied what all three leaned on — the structure that gives meaning something stable to bind to, modelled deliberately for a world where the consumer may be an agent.

Meridian now has its first capsule: a well-modelled client-holdings product, its context bound, its meaning stable, its lineage anchored — one dataset at Level 1, reaching toward Level 2.

But one half of the estate is still ungoverned, and it is the half the copilot reads most. Everything so far — capsule, lakehouse, ML, modelling — assumed the data is *structured*: tables, columns, keys, a grain you can model. Yet the adviser copilot answers at least as many questions from *unstructured* context — research notes, policy PDFs, call transcripts, and the vector index built over them — and none of the structure this chapter built reaches that half. A capsule can bind a definition to a column; it is far less obvious what it binds to a document. The next chapter closes that gap: it extends *Bind* to unstructured data and the retrieval systems that consume it, completing the move across the *whole* estate — before Part III turns hand-binding into platform-automation.

## Further reading

- On dimensional modelling, R. Kimball & M. Ross, *The Data Warehouse Toolkit*; on the normalised/integration tradition, W. H. Inmon's warehouse writing.
- On Data Vault 2.0, D. Linstedt & M. Olschimke, *Building a Scalable Data Warehouse with Data Vault 2.0*.
- On the semantic layer, the dbt MetricFlow / Semantic Layer documentation and the LookML modelling guides — the tooling that carries one governed definition to humans and agents alike (developed in Chapters 16–17).
